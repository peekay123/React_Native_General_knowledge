...
const DemocracyDetail = ({navigation, route}) => {
    ...
    useEffect(() => {
        const getWords = async () => {
            const docRef = doc(db, "Democracy", wordid);
  ...
  return (
        <View style={{flex:1,height:height,width:width}}>
            ...
            <TouchableOpacity style={styles.button} onPress={()=>navigation.navigate('DemocracyActivity', {wordid: wordid,day: day})}> 
              <Text style={styles.wordText}>Attempt Activity</Text>
            </TouchableOpacity>   
        </View>
  )
}
export default DemocracyDetail;
