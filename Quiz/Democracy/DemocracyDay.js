...
import DemocracyWord from '../democompo/DemocracyWord';
import DemocracyWord2 from '../democompo/DemocracyWord2';
import { Ionicons } from '@expo/vector-icons';
import AsyncStorage from '@react-native-async-storage/async-storage';

const {width,height}= Dimensions.get('window');
const DemocracyDay = ({navigation, route}) => {
    ...
    useEffect(() => {
        const collectionRef = collection(db, "Democracy");
        ...
        <ScrollView>
          ...
          <View style={styles.bodyCon}>
            {words.slice(0,counter+1).slice(x,y).map(({ id, day  })=>(
              <DemocracyWord key={id} id={id} day={day} navigation={navigation} />
                ))}
            {words.slice(counter+1,365).slice(x,y).map(({ id, day  })=>(
              <DemocracyWord2 key={id} id={id} day={day}  />
            ))}
          </View>
