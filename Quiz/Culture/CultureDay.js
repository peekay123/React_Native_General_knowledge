...
import CultureWord from '../culcompo/CultureWord';
import CultureWord2 from '../culcompo/CultureWord2';
import { Ionicons } from '@expo/vector-icons';
import AsyncStorage from '@react-native-async-storage/async-storage';

const {width,height}= Dimensions.get('window');
const CultureDay = ({navigation, route}) => {
    ...
    useEffect(() => {
        const collectionRef = collection(db, "Culture");
        ...
        <ScrollView>
          ...
          <View style={styles.bodyCon}>
            {words.slice(0,counter+1).slice(x,y).map(({ id, day  })=>(
              <CultureWord key={id} id={id} day={day} navigation={navigation} />
                ))}
            {words.slice(counter+1,365).slice(x,y).map(({ id, day  })=>(
              <CultureWord2 key={id} id={id} day={day}  />
            ))}
          </View>
